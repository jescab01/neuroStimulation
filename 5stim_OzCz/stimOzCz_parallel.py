...
def stimOzCz_parallel(params, baseline_subj=None):
    ...
    for set in params:
        ...
        if '_pTh' in mode:
            conn = connectivity.Connectivity.from_file(ctb_folder + emp_subj + "_AAL2pTh_end.zip")
        else:
            conn = connectivity.Connectivity.from_file(ctb_folder + emp_subj + "_AAL2_end.zip")
        ...
        if w != 0:
            fft_peaks_hzAAL, fft_peaks_modulesAAL, fft_iafband_modulesAAL = \
                FFTpeaks(raw_data, simLength - transient, initialPeak)
        else:
            fft_peaks_hzAAL, fft_peaks_modulesAAL, fft_iafband_modulesAAL = \
                FFTpeaks(raw_data, simLength - transient)
        ...
        datapoints.append((emp_subj, mode, g, s, r, w,
                           ...
                           fft_peaks_hzAAL,
                           fft_peaks_modulesAAL,
                           fft_iafband_modulesAAL))
