...
import numpy as np
from phasediff_parallel import *
...
freqs, w = [9, 9.5, 10, 10.5, 11, 11.5, 12], 0.03

params = [["isolatedStim_oneNode", subj, 0, 0.11, r, w, f] for r in range(reps) for f in freqs] + \
         [["isolatedStim_twoNodes", subj, 27, 0.11, r, w, f] for r in range(reps) for f in freqs]
...
if rank < size:  # Preventing n_Cores > n_Tasks raising errors
    ...
    local_results = phasediff_parallel(local_params)  # run the function for each parameter set and rank
...
if rank > 0:  # WORKERS _send to rank 0
    ...
    fResults_df = pd.DataFrame(final_results, columns=["mode", "cond", "trial", "node", "weight", "fex",
                                                       "fpeak", "amplitude_fex", "amplitude_fpeak", "plv", "fPhase"])
    ## Save resutls
