...
def stimWfit_parallel(params, baseline_subj=None):
    ...
    for set_ in params:
        ...
        ## Calculate IAF band power rise in the regions of interest
        ...
        amp_fpeak05 = np.array(
            [sum(spectra_filt[i, (freqs_filt > fp - 0.5) & (freqs_filt < fp + 0.5)]) for i, fp in enumerate(fpeak)])
        ...
        datapoints.append((emp_subj, "sigma"+str(sigma), g, r, w,
                           np.average(fpeak), np.average(amp_fpeak05), np.average(amp_fbaseline05)))
