...
import numpy as np
from variabilities_parallel import *
import pickle
...
mode, sigma = "variabilities_cb_indiv", 0.11
...
if rank < size:  # Preventing n_Cores > n_Tasks raising errors
    ...
    local_results = variabilities_parallel(local_params)  # run the function for each parameter set and rank
...
if rank > 0:  # WORKERS _send to rank 0
    ...
    baseline_results = pd.DataFrame(baseline_results, columns=["subject", "mode", "coup", "trial", "w",
                                                               "fpeak", "amp_fpeak", "amp_fbase", "plv",
                                                               "fpeak_rois", "amp_fpeak_rois", "amp_fbase_rois", "plv_rois"])
...
## Define param combinations
# Common simulation requirements
w_space = [0, 0.6] # + list(np.linspace(0.1, 1, 50)) # list(np.logspace(-3, 0, 50))
...
local_results = variabilities_parallel(local_params, baseline_subj)  # run the function for each parameter set and rank
