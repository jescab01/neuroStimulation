...
folder = "E:\LCCN_Local\PycharmProjects\\neuroStimulation\PAPER2_AbstractMode\R2_CingulumBundle\R2b_stimWfit\\stimWfit_SPK\\"

# cargar los datos
stimWfit = pd.read_csv(folder + fname, delimiter="\t", index_col=0)
...
include_w = sorted(set(stimWfit.w))[::]
...
pio.write_image(fig, file=folder + '\\R2a_allNemosAAL_4scatterpercent_alphaRise_' + str(n_trials) + "sim.svg")
pio.write_image(fig, file=folder + '\\R2a_allNemosAAL_4scatterpercent_alphaRise_' + str(n_trials) + "sim.png")

fig.add_trace(go.Scatter(x=w, y=median, mode="lines", name="median", line=dict(color='slategray', width=4), visible="legendonly"))
pio.write_html(fig, file=folder + '\\R2a_allNemosAAL_4scatterpercent_alphaRise_' + str(n_trials) + "sim.html",
               auto_open=True)
...
pio.write_html(fig, file=folder +'\\R2b_allNemosAAL_1scatterModules_alphaRise_' + str(n_trials) + "sim.html",
               auto_open=True)
