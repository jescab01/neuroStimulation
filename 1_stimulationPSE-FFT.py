...
stimulate_indexes = {"AVG_NEMOS_disconn-acc.zip": [0]}
control_idx = [1]
...
for connectome in stimulate_indexes.keys():
    ...
    stim_freqs = np.arange(1, 40, 1)  # np.arange(1, 80, 1)
    stim_amp = np.arange(0, 1, 0.02)  # np.arange(0, 0.05, 0.002)
    ...
    for f in stim_freqs:
        for a in stim_amp:
            ...
            data.append([f, a] + list(avg_activation[stimulate_indexes[connectome]]) + list(avg_activation[control_idx]) + list(fft_peaks[stimulate_indexes[connectome]]) + list(fft_peaks[control_idx]))
