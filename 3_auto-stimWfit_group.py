...
specific_folder = main_folder + "\\PSE_autoGROUP_Wfit-NEMOS" + mode + time.strftime("m%md%dy%Y-t%Hh.%Mm.%Ss")
...
for j, sB in enumerate(secondBlock):
    ...
    while rise_avg < target_rise - 5 or rise_avg > target_rise + 5 or r < 3:
        ...
        for emp_subj, alpha_data in baselineAAL_subj.iterrows():
            ...
            rise = (np.average(fft_band_modulesAAL) - baselineAAL_subj["band_module"][emp_subj]) / \
                   baselineAAL_subj["band_module"][emp_subj] * 100

            temp_secondBlockAAL = temp_secondBlockAAL.append(
                {"subject": emp_subj, "w": w, "peak_hz": np.average(fft_peaks_hzAAL),
                 "peak_module": np.average(fft_peaks_modulesAAL),
                 "band_module": np.average(fft_band_modulesAAL), "percent": rise}, ignore_index=True)
...
for i, w in enumerate(w_space):
    ...
    for r in range(n_rep):
        ...
        for emp_subj, alpha_data in baselineAAL_subj.iterrows():
            ...
            # if ((np.average(fft_band_modulesAAL) - baselineAAL_group["band_module"]) * 100 < 0) & (len(saved_signals) < 10):
            ...
            resultsAAL = resultsAAL.append({"subject": emp_subj, "w": w, "peak_hz": np.average(fft_peaks_hzAAL),
                                            "peak_module": np.average(fft_peaks_modulesAAL),
                                            "band_module": np.average(fft_band_modulesAAL), "percent": rise},
                                           ignore_index=True)
...
# resultsAAL["percent"] = [
#     ((resultsAAL.band_module[i] - baselineAAL_group["band_module"]) / baselineAAL_group["band_module"]) * 100 for i in range(len(resultsAAL))]

resultsAAL_avg = resultsAAL.groupby(['w', 'subject']).mean()
resultsAAL_avg["sd"] = resultsAAL.groupby('w')[['w', 'peak_module']].std()

fig = px.box(resultsAAL, x="w", y="band_module",
             ...
             template="plotly")
pio.write_html(fig, file=specific_folder + '\\allNemosAAL_alphaRise_modules_' + str(n_rep) + "sim.html",
               auto_open=True)
...
fig = px.box(resultsAAL, x="w", y="percent",
             ...
             template="plotly")
pio.write_html(fig, file=specific_folder + '\\allNemosAAL_alphaRise_percent_' + str(n_rep) + "sim.html",
               auto_open=True)
...
pio.write_html(fig, file=specific_folder + '\\allNemosAAL_alphaRise_percentScatter_' + str(n_rep) + "sim.html",
               auto_open=True)
